...
class Captcha {
    protected $config =	array(
        'seKey'     =>  'stivenjoe8.apicaptcha',   // 验证码加密密钥
     ...
     * 验证验证码是否正确
     * @access public
     * @param string $ucode 用户输入的验证码
     * @param string $verify_code 对应verifyid的验证码
     * @return bool 用户验证码是否正确
     */
    public function check($ucode, $verify_code) {
        ...
        //对比
        if($this->authcode(strtoupper($ucode)) == $verify_code) {
            return true;
        }
     ...
     * @return mixed
     */
    public function entry($verifyid = '') {
        // 图片宽(px)
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        // 保存验证码
		$verifyid 	= 	trim($verifyid) ? $verifyid : uniqid(); 
        ...
        //图像base64
        $secode['verify_img'] = base64_encode($content);
		
		//返回结果数组，可以将verify_key:verify_code缓存；将verifyid和verify_img通过接口返回
        return $secode;
    }
